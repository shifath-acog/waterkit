...
def _write_pdb_file(output_name, molecule, overwrite=True, **kwargs):
    ...
    for residue in molecule.residues:
        ...
        for atom in residue.atoms:
            ...
            # Change 1 by Kailash, above commented out line[102] is replace with below if else code line[104:107] 
            if len(atom.name) >= 2 and atom.name[:2].capitalize() in ["Br", "Cl", "Na", "Ca", "Fe", "Zn"]:
                atom_type = atom.name[:2].capitalize()  # Get first two letters
            else:
                atom_type = atom.name[0]  
...
class PrepareReceptor:
    ...
    def prepare(self, pdb_filename, lib_files=None, frcmod_files=None, clean=True):
        ...
        # Create Amber topology and coordinates files  # change 2 by kailash
        # with utils.temporary_directory(prefix='wk_preparation_', dir='.', clean=clean) as tmp_dir:   # original
        with utils.temporary_directory(prefix='wk_preparation_', dir='.', clean=False) as tmp_dir:     # replaced
            if lib_files is not None:
            ...
            # change 3 by kailash, lines [901:916] replaced with [918:934]
            ...
            with open(tleap_input, 'w') as w:
                content = _make_leap_template(
                    pdbfixer.parm, final_ns_names, gaplist, sslist,
                    input_pdb=pdb_clean_filename,
                    prmtop=prmtop_filename, rst7=rst7_filename,
                    lib_files=local_lib_files, frcmod_files=local_frcmod_files
                )
            ...
            try:
                debug_file_path = os.path.join(os.getcwd(), 'tleap_input_debug.txt')
                with open(debug_file_path, 'w') as debug_file:
                    debug_file.write(content)
                print(f"TLEaP input content saved to {debug_file_path}")
            except Exception as e:
                print(f"Failed to save TLEaP input content: {str(e)}")
